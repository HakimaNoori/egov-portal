...
export default function OfficerRequestsList() {
  ...
  return (
    <div className="p-6">
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-800">Requests (Officer)</h2>
      </section>
                ...
                {requests.map((r) => (
                  <tr key={r.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">
                      <Link to={`/officer/requests/${r.id}`} className="px-3 py-1 border rounded-lg hover:bg-gray-100">View</Link>
                    </td>
                  </tr>
                ))}
                {requests.length === 0 && (
                  <tr>
                    <td colSpan="5" className="px-3 py-4 text-center text-gray-500">No requests</td>
                  </tr>
                )}
              </tbody>
            </table>
